...
imagens = {
    1: carregar_imagem("pedra"),
    2: carregar_imagem("papel"),
    3: carregar_imagem("tesoura"),
}


running = True
game_over = False
mensagem = "escolha a sua opcao: p(pedra), a(papel), t(tesoura)"
pontos_computador = 0
pontos_jogador = 0
opcao_maquina = None
opcao_jogador = None
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if (
            ...
            and pontos_jogador < 3
            and pontos_computador < 3
        ):
            tecla = event.unicode
    ...
    texto_pontuacao_pc = fonte_pequena.render(
        f"Computador: {pontos_computador}", True, (255, 0, 0)
    )
    texto_pontuacao_jogador = fonte_pequena.render(
        f"Jogador: {pontos_jogador}", True, (255, 0, 0)
    )
